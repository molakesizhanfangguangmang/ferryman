...
不判断内容，不重试，不排队。收到什么转什么，一条一条来，失败就如实报失败。唯一的例外是 `DROP_KEYWORDS`（默认关），那也只是一条「正文里含不含这几个词」的判断。
...
有些通知你知道不必转。bili-sync 的「N 条新视频已入库」就是：那说的是它的五道子任务状态位，跟文件落没落盘是两件事，真正的「下载完成」另有人推。与其在下游再滤一遍，不如进来说一句就丢掉。

`DROP_KEYWORDS` 填逗号分隔的词。抽出来的正文里含其中任何一个，整条吞掉，不往目标发：

```ini
DROP_KEYWORDS=已入库
```

- 子串匹配，区分大小写，不当正则用；留空（默认）= 不过滤。
- 匹配的是**抽出来的正文、截断之前**，所以 `MAX_TEXT_LEN` 调小不会把该丢的漏掉。
- 吞掉的请求回 `{"ok":true,"dropped":true,"keyword":"..."}`，HTTP 200，日志里留一行——发送方不会当成失败去重发。
- 这个词表跟目标类型无关，五档都吃。
...
| 某几条日志是 `dropped` | 命中了 `DROP_KEYWORDS`，本就该吞。不想吞就把词去掉 |
