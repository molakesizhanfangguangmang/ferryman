...
**一个极小的 webhook 中转：把收到的请求抽成一条纯文本，按你配的目标发出去。**

没有 Web 界面，没有数据库，没有队列，没有第三方依赖。整个项目就是一个 Python 文件加一个安装脚本，跑成一个容器。

名字来自摆渡人：东西从这一岸到那一岸，它只负责摆渡，不关心乘客是谁。

## 轻到什么程度

| 项 | 值 |
| --- | --- |
| 第三方依赖 | **0**。纯 Python 标准库，`import` 的全是标准库 |
| 代码量 | `relay.py` 单文件约 **400 行**，含注释 |
| 镜像 | 直接用官方 `python:3.12-alpine`。项目里没有 Dockerfile，不自己构建 |
| 配置 | 一个文本文件 `relay.env`，六七行 |
| Web 界面 | **没有** |
| 数据库 / 队列 / 定时任务 / 状态文件 | **都没有**。进程不落任何数据，重启不留痕 |
| 后台进程 | 一个 HTTP server，占一个 TCP 端口 |
| 安装 | 一条命令 |
| 卸载 | 一条命令 |

整个仓库就是四个文件加这份 README：一个 py、一个安装脚本、一个卸载脚本、一个配置示例。

**为什么不做 Web 界面**：配置就那几项，多一个前端就多一份依赖、一份资源和一份攻击面。要看状态用 `docker logs`、`curl` 健康检查、`docker ps`；要改配置改文件再 `docker compose up -d --force-recreate`。目标不是做一个平台，是做一个下班前五分钟能装完、装完就不用管的转发件。

对比那些能拖流程的转换层（n8n、Node-RED、Huginn）：它们能做的事比这个多得多，代价是自带前端和数据库、镜像和常驻开销都高出量级。ferryman 只做转发这一件事。

## 什么时候需要它，什么时候不需要

**不需要**：目标服务能直接吃源服务发的东西。比如 bili-sync 原生支持 Telegram / SMTP，配一个通知器就完事，中间不用加东西。

**需要**（三种情况，中一条就够）：

1. 源服务发的 body 是「多行文本塞在 JSON 壳里」，接收方解析不了。bili-sync 的默认模板 `{"text": "{{{message}}}"}` 就是这样——通知正文带真换行，发出去是非法 JSON，接收方直接 422。
2. 源服务只会 POST 一个 URL，而你要推到它不认识的地方（QQ、微信、自建 bot、另一个中转）。
3. 源服务发完不看状态码、失败一声不吭；你想有一层能看日志的。

## 它做什么、不做什么

**做**：收 POST（GET 是健康检查）；抽文本——JSON 壳里挑 `text` / `message` / `content` / `msg` / `desp` / `body`（含一层嵌套），裸文本直接用，字符串数组按行拼，像 HTML 的粗剥标签；把非法 JSON 里的裸换行修回来；超长截断；按目标类型拼成合法请求发出去；结果写日志；返回 200 或 502。

**不做**：不判断内容、不过滤、不重试、不排队。收到什么转什么，一条一条来，失败就如实报失败。

## 安装

三种填法，挑一种。**在终端里跑会直接问**；非终端环境（脚本、CI、管道）不会卡在提问上，而是生成 `relay.env` 让你填或补命令行参数。
...
参数没给全的项沿用 `relay.env` 里的旧值；`--ask` 强制走问答（会先把旧配置备份成 `relay.env.bak`）。全部选项：`bash install.sh --help`。

安装脚本会做：校验配置是否齐全 → 检查端口是否被占（端口撞了就停下让你换，不硬上） → 写 `docker-compose.yml` → **自检**（用你的配置真发一条测试消息，收到才算过） → 起容器。自检不过就不启动，改配置再重跑；`relay.env` 和 compose 都还在，不用重填。
...
- **host 网络**：唯一原因是要走 `127.0.0.1` 打到宿主上的服务。不接本机目标的话，改回 bridge 也行。
- **每次请求单独的线程**：标准库 `ThreadingHTTPServer`，一条慢请求不堵后面的。
